/****************************************************************************
** $Id: qlocalfs.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
void QLocalFs::operationListChildren( QNetworkOperation *op )
{
	...
	infos << QUrlInfo( fi->fileName(), convertPermissions(fi), fi->owner(), fi->group(),
        ...
        // ### Block signals to initiate a hack in QNetworkOperation that will
        // prevent the op from being deleted during the qApp->processEvents() call
        op->blockSignals( TRUE );
        qApp->processEvents();
        op->blockSignals( FALSE );
        if ( op->state() == StStopped ) {
            op->free();
            return;
        }
    }
