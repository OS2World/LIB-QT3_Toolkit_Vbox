/****************************************************************************
** $Id: qdial.cpp 174 2007-11-06 22:27:57Z dmik $
**
...
void QDial::repaintScreen( const QRect *cr )
{
    ...
    // note: drawing dial controls is NOT AT ALL using the QStyle drawing
    // concept, so we have to draw the Warp4 dial here explicitly in
    // the widget code!
    if ( style().styleHint( QStyle::SH_GUIStyle ) == QStyle::PMStyle ) {
        QRect rr(rect());

        // erase the background
        QBrush br( paletteBackgroundColor() );
        p.setPen( QPen::NoPen );
        p.fillRect( rr, br );

        // centered quadratic rectangle
        if ( rr.width() > rr.height() ) {
            rr.setWidth( rr.height() );
            rr.moveBy( (width() - rr.width()) / 2, 0 );
        } else {
            rr.setHeight( rr.width() );
            rr.moveBy( 0, (height() - rr.height()) / 2 );
        }

        // draw the ticks ("notches")
        if ( d->showNotches ) {
            calcLines();
        ...
        // draw dial (circle with shadow)
        int bigLineSize = calcBigLineSize();
        int dist = 6;
        rr.setLeft( rr.left() + bigLineSize + dist );
        rr.setTop( rr.top() + bigLineSize + dist );
        rr.setRight( rr.right() - bigLineSize - dist );
        rr.setBottom( rr.bottom() - bigLineSize - dist );
        p.setPen( QPen( colorGroup().shadow(), 2) );
        p.drawEllipse( rr );
        rr.setLeft( rr.left() + 2 );
        rr.setTop( rr.top() + 2 );
        rr.setRight( rr.right() - 2 );
        rr.setBottom( rr.bottom() - 2 );
        QPointArray pa;
        pa.putPoints( 0, 3,
                      rr.left() - 2, rr.top() - 2,
                      rr.right() + 2, rr.top() - 2,
                      rr.left() - 2, rr.bottom() + 2 );
        p.setClipRegion( QRegion( pa ) );
        p.setPen( QPen( colorGroup().light(), 2 ) );
        p.drawEllipse( rr );
        pa.resize( 0 );
        pa.putPoints( 0, 3,
                      rr.right() + 2, rr.top() - 2,
                      rr.right() + 2, rr.bottom() + 2,
                      rr.left() - 2, rr.bottom() + 2 );
        p.setClipRegion( QRegion( pa ) );
        p.setPen( QPen( colorGroup().dark(), 2 ) );
        p.drawEllipse( rr );
        p.setClipping( false );
        ...
        double s = sin( a ),
               c = cos( a );
        int r = rr.width() / 2 - 9;
        double xtr = ((double)rr.left() + rr.right()) * .5 + r * c,
               ytr = ((double)rr.top() + rr.bottom()) * .5 - r * s;
        QPointArray ar;
        ar.putPoints( 0, 3,
                      (int)(xtr + 3.5 * c), (int)(ytr - 3.5 * s),
                      (int)(xtr + 4 * cos(a - 2.0944)), (int)(ytr - 4 * sin(a - 2.0944)),
                      (int)(xtr + 4 * cos(a + 2.0944)), (int)(ytr - 4 * sin(a + 2.0944)) );
