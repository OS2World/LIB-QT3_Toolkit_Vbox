/****************************************************************************
** $Id: qpainter.h 61 2006-02-06 21:43:39Z dmik $
**
...
private:
    ...
    void	drawPolyInternal( const QPointArray &, bool close=TRUE,
                                  bool winding=FALSE, int index=0, int npoints=-1,
                                  bool disjoint=FALSE );
    enum        ArcClose { CloseNone, CloseChord, ClosePie };                                      
    void        drawArcInternal( int x, int y, int w, int h, int a = 0,
                                 int alen = 360*16, ArcClose close = CloseChord );
    void        drawWinFocusRectInternal( int x, int y, int w, int h,
                                          const QColor &bgCol, bool xormode );
    void        drawRectInternal( int x, int y, int w, int h, int wRnd, int hRnd ); 
#else    
    void	drawPolyInternal( const QPointArray &, bool close=TRUE );
#endif    
...
#elif defined(Q_WS_PM)
    bool setNativeXForm( bool assumeYNegation );
    void clearNativeXForm();
    friend class QFontEnginePM;
    HPS		hps;
    HPAL	holdpal;
    HRGN	holdrgn;
    HBITMAP	hbrushbm;
...
#elif defined(Q_WS_PM)
inline HPS QPainter::handle() const
{
    return hps;
}
#elif defined(Q_WS_X11) || defined(Q_WS_MAC)
