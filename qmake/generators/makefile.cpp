/****************************************************************************
** $Id: makefile.cpp 83 2006-04-15 15:29:11Z dmik $
**
...
{
    //@@TODO (dmik): OS/2 cannot create multiple subdirs in mkdir.
    //@@TODO (dmik): fix it here? (mkdir a\b\c = mkdir a;a\b;a\b\c)
	...
	if(!inc.isEmpty()) {
		...
		if((Option::target_mode == Option::TARG_MAC9_MODE && inc.find(':')) ||
		   ((Option::target_mode == Option::TARG_WIN_MODE ||
                     Option::target_mode == Option::TARG_OS2_MODE) && inc[1] != ':') ||
...
#if defined(Q_OS_OS2)
    // As far as I understand, this code tries to load "qt-mt" prl if "qt" is
    // specified, but not found, and vice versa. First of all, I don't get why
    // we should do that (the user has specified he wants a particular single-
    // threaded or multi-threadad version using the "thread" CONFIG keyword).
    // And more over, the -mt suffix is no more used on OS/2.
#else    
...
MakefileGenerator::create(QMakeProject *proj)
{
	...
	mkfile = new GNUMakefileGenerator(proj);
