/****************************************************************************
** $Id: qdir.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
/*!
    ...
    (optionally preceded by a drive specification under Windows and OS/2). If
    ...
    Windows or OS/2 the list will usually contain "C:/", and possibly "D:/",
    etc.
...
#if defined(Q_OS_OS2)
#if defined(Q_CC_GNU)
	char drv = fileName.upper()[0].latin1();
	if ( _getdrive() != drv ) {
            char buf[PATH_MAX];
            _getcwd1( buf, drv );
#else
...
/*!
    ...
    On OS/2, convertSeparators("c:/os2/system") returns
    "c:\os2\system".
...
/*!
    ...
    Mac OS X) and "\" under Windows and OS/2.
...
/*!
    ...
    Under OS/2 the \c HOME environment variable is also used. If it
    does not exist the path is formed by concatenating the \c
    HOMEDRIVE and \c HOMEPATH environment variables. If they don't
    exist the rootDirPath() is used (which always returns "x:/" where \c x
    is the system boot drive letter).
