/****************************************************************************
** $Id: qregion.h 61 2006-02-06 21:43:39Z dmik $
**
...
public:
   ...
   ~QRegion();
    ...
    HRGN handle( int target_height = 0 ) const {
        if ( !data->rgn || data->hgt != target_height )
            updateHandle( target_height );
        return data->rgn;
    }
...
#elif defined(Q_WS_PM)
    QRegion( HRGN hrgn, int target_height );
    QRegion pmCombine( const QRegion &, int ) const;
    void updateHandle( int target_height ) const;
#elif defined(Q_WS_X11)
