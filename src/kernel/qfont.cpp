/****************************************************************************
** $Id: qfont.cpp 80 2006-04-06 19:32:24Z dmik $
**
...
// On OS/2, destructors of static objects in a DLL cannot use objects created
// on the stack of the main thread when the application is being abnormally
// terminated (exit(), DosExit(), DosKillProcess(), etc), because at a time when
// these static destructors are called (from _DLL_InitTerm on most compilers,
// including GCC and IBM VAC) the stack is most likely reused  by the DLL
// termination code. Since QFontCache is a child of QApplication (which is
// usually created as a local object inside the main() function) and wants to
// use its parent during destruction, this becomes a real problem and causes a
// guaranteed application trap during abnormal termination. The workaround is to
// use atexit() instead. Note that this will only work for the exit() case
// (on DosExit() or DosKillProcess(), the stdlib's exit list is not processed).

static void cleanupFontCache()
{
    if (QFontCache::instance != 0)
...
#if defined(Q_WS_WIN) || defined(Q_WS_PM) || defined (Q_WS_QWS)
