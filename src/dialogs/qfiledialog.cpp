/****************************************************************************
** $Id: qfiledialog.cpp 185 2008-04-03 20:39:42Z dmik $
**
...
void QFileDialog::insertEntry( const QValueList<QUrlInfo> &lst, QNetworkOperation *op )
{
    ...
    // '..' may be marked as hidden in directories that are one level away from
    // root on some file systems and/or drives, but we never want to hide '..'
	if ( !bShowHiddenFiles && inf.name() != ".." ) {
	    if ( d->url.isLocalFile() ) {
		QString file = d->url.path();
		if ( !file.endsWith( "/" ) )
