/****************************************************************************
** $Id: qmessagebox.cpp 21 2005-11-17 19:51:23Z dmik $
**
...
#endif
#if defined(Q_WS_PM)
//@@TODO (dmik): this is necessary for WinAlarm() which will be
//  probably removed from here later.
#include "qt_os2.h"
#endif
...
#else
#if defined(Q_WS_PM)
//@@TODO (dmik): currently, the accessibility support is not implemented
//  on OS/2, but we still need to play a sound when QMessageBox is opened.
    ULONG alarmStyle = 0;
    bool playAlarm = TRUE;
    switch ( icon() ) {
        case Warning:
            alarmStyle = WA_WARNING;
            break;
        case Critical:
            alarmStyle = WA_ERROR;
            break;
    ...
    if ( playAlarm )
        WinAlarm( HWND_DESKTOP, alarmStyle );
#endif
#endif
    QDialog::showEvent( e );
}
