/****************************************************************************
** $Id: qglobal.cpp 173 2007-11-06 22:00:00Z dmik $
**
** Global functions
**
** Created : 920604
...
/*!
    ...
    Under X11 or OS/2, the text is printed to stderr. Under Windows, the text
...
// OS/2 PmPrintf debug output function

static
void qPmPrint( const char *buf )
{
    // recognized environment variables:
    // QT_PM_DEBUG_FILE=<filename>   output to file
    // QT_PM_DEBUG_QUEUE             output to queue \QUEUES\PRINTF32
    //                               that can be read e.g. by PmPrintf
    // <nothing>                     output to stderr

    char const *queue_env = ::getenv( "QT_PM_DEBUG_QUEUE" ),
               *file_env = ::getenv( "QT_PM_DEBUG_FILE" );

    if ( queue_env != 0 ) {
        PID srvPid;
        HQUEUE queue;
        if ( ::DosOpenQueue( &srvPid, &queue, "\\QUEUES\\PRINTF32" ) == NO_ERROR ) {
            char* shrBuf; 
            unsigned long bufSize = ::strlen(buf) + 1;
            if ( ::DosAllocSharedMem( (PPVOID) &shrBuf, NULL, bufSize,
                                      OBJ_GIVEABLE | PAG_WRITE |
                                      PAG_COMMIT ) != NO_ERROR ) {
                ::DosSleep( 200 ); // give it a second chance...
                if ( ::DosAllocSharedMem( (PPVOID) &shrBuf, NULL, bufSize,
                                          OBJ_GIVEABLE | PAG_WRITE |
                                          PAG_COMMIT ) != NO_ERROR ) {
                    ::DosCloseQueue( queue );
            ...
            if ( ::DosGiveSharedMem( shrBuf, srvPid, PAG_READ ) == NO_ERROR ) {
                time_t tt;
                ::time(&tt);
                ::DosWriteQueue( queue, (ULONG)tt, bufSize, shrBuf, 0 );
            }
...
/*!
  ...
  Under OS/2, the value -1 for \a code should only be used after Presentation
  Manager calls.
...
/*!
    ...
    output under X11 and OS/2 or to the debugger under Windows. If it is a
