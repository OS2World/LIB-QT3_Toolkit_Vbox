/****************************************************************************
** $Id: qpopupmenu.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
void QPopupMenu::drawContents( QPainter* p )
{
	...
	if ( ncols > 1 && y + itemh - 1 > contentsRect().bottom() ) {
	    if ( y <= contentsRect().bottom() ) {
		QRect rect(x, y, itemw, contentsRect().bottom() - y + 1);
		if(!p->hasClipping() || p->clipRegion().contains(rect)) {
...
#endif
//@@TODO (dmik): we need to play a sound when a disabled item is activated
//  to conform to the OS/2 CUA. Btw, should'n it be supported by QAccessibility
//  globally in Qt?
...
#if !defined(Q_WS_PM) // do it later, only in case of item activation
		byeMenuBar();
#endif
#ifndef QT_NO_WHATSTHIS
		bool b = QWhatsThis::inWhatsThisMode();
...
#if defined(Q_WS_PM)
//@@TODO (dmik): we need to play a sound when the disabled item is activated
//  to conform to the OS/2 CUA. Btw, should'n it be supported by QAccessibility
//  globally in Qt?
                else
                    WinAlarm( HWND_DESKTOP, WA_ERROR );
