/****************************************************************************
** $Id: qtooltip.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
#ifdef Q_WS_PM    
    connect( &doAllowAnimation, SIGNAL(timeout()), SLOT(allowAnimation()) );
#endif    
...
{
#ifdef Q_WS_PM
    // OS/2 PM has a very low limit on the number of simultaneous PM timers,
    // so we reuse the same timer instead of creating a new one every time this
    // function is invoked (it can happen more frequently than once per 250 ms).
    doAllowAnimation.start( 250, TRUE );
#else
