/****************************************************************************
** $Id: qdragobject.cpp 96 2006-07-06 21:11:46Z dmik $
**
...
const char * staticCharset(int i)
{
    ...
    switch ( i ) {
#if defined(Q_WS_PM)
      // We want "text/whatever" w/o any charset to be the first (the "default")
      // mime type of the QTextDrag object, so that a rendering mechanism/format
      // pair for it will be the first in the RMF list, wchich is essential for
      // some native drop targets (i.e. WPS) to render text correctly. Note that
      // it won't hide unicode data from Qt apps because QTextDrag::decode()
      // for Q_WS_PM prefers unicode mime types (see the implementation below). 
      case 0:
	return "";
      case 1:
...
bool QTextDrag::decode( const QMimeSource* e, QString& str, QCString& subtype )
{
...
#if defined(Q_WS_PM)
    // We give Unicode formats a higher priority, so that if there are several
    // encodings of the same subtype present in the given mime source, a
    // Unicode one will be used for decoding. Otherwise, the first recognized
    // format will be choosen.
    int fallbackMime = -1;
    ...
    for( int i = 0; /* FOREVER */; ++i ) {
        mime = e->format( i );
        if ( !mime ) {
            // We went through all mimes but didn't find Unicode. If there is
            // a fall-back mime, iterate again starting directly from it.
            if ( !secondPass && fallbackMime >= 0 ) {
                i = fallbackMime - 1; // will do ++i on continue
                secondPass = TRUE;
                continue;
            }
            break;
	...
	if ( 0==qstrnicmp(mime,"text/",5) ) {
		...
		if (html)
		    ...
		    codec = codecForHTML(QCString(e->encodedData(mime)));
		if (!codec)
		    codec = qt_findcharset(m);
		if ( codec ) {
#if defined(Q_WS_PM)
                    if ( codec->mibEnum() == 1000 /* ISO-10646-UCS-2 */ ||
                         codec->mibEnum() == 106 /* UTF-8 */ ) {
                        // no need to try it again on the second pass
                        if ( secondPass )
                            continue;
                    } else if ( !secondPass ) {
                        // memorize the fall-back mime on the first pass
                        if ( fallbackMime < 0 )
                            fallbackMime = i;
                        continue;
                    }
