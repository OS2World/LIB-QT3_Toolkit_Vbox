/****************************************************************************
** $Id: qwidget.cpp 102 2006-07-24 21:11:03Z dmik $
**
...
#ifdef Q_WS_PM
    setWindowState(WindowMinimized);
#else
...
//	&& WinQueryFocus( HWND_DESKTOP ) == winId()
#endif
	)
	return;
...
//  to completely handle focus traversal itself. 
//	if ( !topLevelWidget()->isPopup() )
//	    WinSetFocus( HWND_DESKTOP, winId() );
//	else {
#endif
#if defined(QT_ACCESSIBILITY_SUPPORT)
...
//@@TODO (dmik): currently we don't use WinSetFocus(). what for? Qt seems
//  to completely handle focus traversal itself. 
//	if ( !isPopup() && WinQueryFocus( HWND_DESKTOP ) == winId() )
//	    WinSetFocus( HWND_DESKTOP, topLevelWidget()->winId() );
//	else {
#endif
#if defined(QT_ACCESSIBILITY_SUPPORT)
	    QAccessible::updateAccessibility( this, 0, QAccessible::Focus );
...
#endif
//@@TODO (dmik): I think we don't need the same as above in OS/2,
//  because if the window is active (in Qt terminology)
//  qApp->activeWindow() will be equal to toplevelWidget() which is
//  checked at the beginnig.
...
void QWidget::show()
{
	...
	bool wasMax = windowState() & WindowMaximized;
	bool wasFullScreen = windowState() & WindowFullScreen;
	if ( !s.isEmpty() )
	    resize( s );
	if (wasMax)
...
//@@TODO (dmik): need the same in OS/2? guess not.        
#if defined(Q_WS_WIN)
...
//@@TODO (dmik): is it correct for OS/2?    
#if defined(Q_WS_WIN) || defined(Q_WS_PM)
...
#ifndef Q_WS_PM
    // We disable this on OS/2 since we send these events from qt_sendBlocked()
    // which does it more accurately compared to the code below -- only those
    // widgets that have actually been blocked/unblocked (not less and not
    // more) will receive these events.         
