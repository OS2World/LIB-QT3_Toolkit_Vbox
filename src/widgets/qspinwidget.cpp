...
void QSpinWidget::mousePressEvent( QMouseEvent *e )
{
    ...
    // With Warp, the up and down buttons are diagonally separated,
    // so we are disabling the down button entirely and have to find
    // out the position of the click within the up button in order
    // to find out what the user wants!
    int guiStyle = style().styleHint( QStyle::SH_GUIStyle );
    if ( guiStyle == QStyle::PMStyle ) {
        if ( d->up.contains( e->pos() ) ) {
            QPoint rp = e->pos() - d->up.topLeft();
            if ( d->up.width() > (rp.x() + rp.y()) ) {
                if ( d->upEnabled )
                    d->buttonDown = 2;
            } else {
                if ( d->downEnabled )
	...
	if ( !d->buttonDown ) {
	    repaint( d->down.unite( d->up ), FALSE );
            ...
            // do all with the "up" button in PM style - see note above
            if ( guiStyle == QStyle::PMStyle )
                repaint( d->up, FALSE );
            else
...
void QSpinWidget::mouseReleaseEvent( QMouseEvent *e )
{
    ...
    if ( oldButtonDown != d->theButton ) {
        // do all with the "up" button in PM style -
        // see note in mousePressEvent
        int guiStyle = style().styleHint( QStyle::SH_GUIStyle );
        if ( guiStyle == QStyle::PMStyle )
            repaint( d->up, FALSE );
        else if ( oldButtonDown & 1 )
	    repaint( d->down, FALSE );
...
void QSpinWidget::mouseMoveEvent( QMouseEvent *e )
{
    ...
    if ( guiStyle == QStyle::PMStyle ) {
        ...
        if ( d->down.contains(e->pos()) )
            clickButton = 1;
        else if ( d->up.contains(e->pos()) )
            clickButton = 2;
    }

    uint oldButtonDown = d->theButton;
    if ( (oldButtonDown & 1) && !(clickButton & 1) ) {
	...
	repaint( d->down, FALSE );
    } else if( (oldButtonDown & 2) && !(clickButton & 2) ) {
    //} else if ( oldButtonDown & 2 && !d->up.contains( e->pos() ) ) {
	d->stopTimer();
	d->theButton = 0;
	repaint( d->up, FALSE );
    } else if ( !oldButtonDown && (clickButton & 2) && (d->buttonDown & 2) ) {
    //} else if ( !oldButtonDown && d->up.contains( e->pos() ) && d->buttonDown & 2 ) {
	d->startTimer( 500 );
	d->theButton = 2;
	repaint( d->up, FALSE );
    } else if(!oldButtonDown && (clickButton & 1) && (d->buttonDown & 1)) {
