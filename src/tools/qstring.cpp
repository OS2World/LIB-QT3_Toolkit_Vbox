/****************************************************************************
** $Id: qstring.cpp 169 2007-03-31 16:22:28Z dmik $
**
...
/*!
    ...
    this is the QTextCodec::codecForLocale(). On Windows and OS/2, it is a
...
#endif
#if defined(Q_OS_OS2)
    return isNull() ? QCString("") : qt_os2QString2MB( *this );
#endif
#ifdef Q_WS_QWS
...
#endif
#if defined(Q_OS_OS2)
    return qt_os2MB2QString( local8Bit, len );
#endif
#ifdef Q_WS_QWS
    return fromUtf8(local8Bit,len);
#endif
...
int QString::localeAwareCompare( const QString& s ) const
{
    ...
    LocaleObject lo = qt_os2DefLocaleObj();
    if (lo)
        return UniStrcoll( lo, (UniChar*) ucs2(), (UniChar *) s.ucs2() );
    else
        return ucstrcmp( *this, s );
...
static LocaleObject qt_os2DefLocaleObj_var = NULL;
static UconvObject qt_os2DefUconvObj_var = NULL;

const LocaleObject qt_os2DefLocaleObj()
{
    if ( !qt_os2DefLocaleObj_var )
        UniCreateLocaleObject( UNI_UCS_STRING_POINTER, (UniChar *)L"",
                &qt_os2DefLocaleObj_var );
    return qt_os2DefLocaleObj_var;
}

const UconvObject qt_os2DefUconvObj()
{
    if ( !qt_os2DefUconvObj_var )
        UniCreateUconvObject( (UniChar *)L"", &qt_os2DefUconvObj_var );
    return qt_os2DefUconvObj_var;
}

QCString qt_os2QString2MB( const QString& s, int uclen )
{
    if ( uclen < 0 || (uint) uclen > s.length() )
	uclen = s.length();
    if ( s.isNull() )
	return QCString();
    if ( uclen == 0 )
    ...
    UniChar *ucPtr = (UniChar *) s.unicode();
    size_t ucLeft = uclen;

    // be optimistic (imply that one byte is necessary per every Unicode char)
    size_t mblen = uclen;
    QCString mbBuf( mblen + 1 );
    char *mbPtr = mbBuf.data();
    size_t mbLeft = mblen;
    
    size_t nonIdent = 0;
    UconvObject uo = qt_os2DefUconvObj();
    int rc;
    ...
    while ( ucLeft ) {
        ...
        if ( rc == ULS_BUFFERFULL ) {
            ...
            if ( ucDone ) {
                ...
                newLen = mbDone + (ucLeft * mbDone) / ucDone;
            }
            if ( newLen == mblen ) {
                // could not process a single Unicode char, double the size
                mblen *= 2;
            } else {
                mblen = newLen;
            }
...
#ifndef QT_NO_DEBUG
	    // Fail.
	    qWarning("UniUconvFromUcs cannot convert multibyte text (error %d): %s (UTF8)",
		rc, s.utf8().data());
#endif
            break;
        }
    }
    mbBuf.truncate( mblen - mbLeft );
    return mbBuf;
}

// WATCH OUT: mblen must NOT include the NUL (in contrast with qt_winMB2QString)
QString qt_os2MB2QString( const char* mb, int mblen )
{
    if ( !mb )
	return QString::null;
    if ( mblen < 0 ) {
        ...
        char *end = (char *) memchr (mb, '\0', mblen);
        if ( end )
            mblen = end - mb; 
    }
    if ( !mblen )
    ...
    size_t uclen = mblen;
    QMemArray<UniChar> ucBuf( uclen );
    UniChar *ucPtr = ucBuf.data();
    size_t ucLeft = uclen;
    ...
    while ( mbLeft ) {
        rc = UniUconvToUcs ( uo, (void**)&mbPtr, &mbLeft, &ucPtr, &ucLeft, &nonIdent );
        if ( rc == ULS_BUFFERFULL ) {
            size_t ucDone = uclen - ucLeft;
            size_t mbDone = mblen - mbLeft;
            // assume that mbLeft/ucLeft is an approximation of mbDone/ucDone
            uclen = ucDone + (mbLeft * ucDone) / mbDone; 
            ucBuf.resize( uclen );
            ucPtr = ucBuf.data() + ucDone;
        } else if ( rc != ULS_SUCCESS ) {
#ifndef QT_NO_DEBUG
	    // Fail.
	    qWarning("UniUconvFromUcs cannot convert multibyte text (error %d)", rc);
#endif
            break;
        }
    }
    ...
    if ( uclen == 0 )
	return QString::null;
    QString s( (QChar*)ucBuf.data(), uclen );
    return s;
}

const char *qt_os2UconvFirstByteTable()
{
    static char achFirst[ 256 ] = { -1, };

    if( achFirst[ 0 ] == -1 )
        UniQueryUconvObject( qt_os2DefUconvObj(), NULL, 0, achFirst, NULL, NULL );
