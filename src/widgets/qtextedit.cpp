/****************************************************************************
** $Id: qtextedit.cpp 106 2006-07-29 13:56:44Z dmik $
**
...
#if !defined(Q_OS_MACX) && !defined(Q_OS_OS2)
		  // AltGr+key can produce valid characters in some kbd layouts
		  // (i.e. the German one) which we must not ignore
		  !( e->state() & AltButton ) &&
#endif
		  !( e->state() & MetaButton ) ||
		  // Note (dmik): the below line is possibly a typo because it's
		  // equivalent to just (e->state() & ControlButton) which obviously
		  // contradicts !( e->state() & ControlButton ) above. Anyway,
		  // as a result, Ctrl+key are successfully handled by QTextEdit on
		  // all platforms (because of the || operator), unless assigned 
		  // as hot keys (shortcuts). 
...
#if defined (Q_WS_PM)
    // QTextEdit has WResizeNoErase and WRepaintNoErase flags (actually applied
    // to the viewport()), so we must erase areas not covered by the text doc.
    // [Win32 version feels ok without this, because it doesn't actually
    // fully obey WResizeNoErase and WRepaintNoErase: WM_ERASEBKGND always
    // erases the background before WM_PAINT and after every resize].
    
    int tw = td->width();
    // calculate the height the text doc will occupy starting with clipy
    int th = td->height() - offset;  
    if ( clipx + clipw > td->width() ) {
        // erase background to the left of every line
        p->eraseRect( tw, clipy, clipx + clipw - tw, th );
    }
    if ( cliph > th ) {
        // erase background under the text doc
        p->eraseRect( clipx, clipy + th, clipw, cliph - th );
    }
#endif
