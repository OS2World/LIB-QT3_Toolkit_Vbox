/**********************************************************************
** $Id: qcanvas.cpp 74 2006-03-26 14:09:02Z dmik $
**
...
// QT_NO_CANVAS_ROUNDING_FIX macro encloses some hacks to QCanvas intended
// to get rid of artefacts and distortions caused by rounding errors. These
// hacks give much better results when the canvas is scaled, sheared or
// rotated in the canvas view; however they are just quick patches and not
// perfect -- there are still some artefacts possible in rare cases. As far
// as I understand, there is a general problem in the QCanvas design: it
// operates on integers (instead of doubles) when doing series of direct and
// inverse transformations in a row, that results in a big inaccuracy (more
// than one pixel in some cases).
//
// So far, this fix is tested on OS/2 only, so it is disabled for other
// platforms.

#if !defined(Q_WS_PM)
#define QT_NO_CANVAS_ROUNDING_FIX
#endif

#if !defined(QT_NO_TRANSFORMATIONS) && !defined(QT_NO_CANVAS_ROUNDING_FIX)
#include <math.h>
#endif
 ...
 * \internal
 *
 * Takes inclusive top left and exclusive bottom right poitns of the given
 * rectangle, translates it using the given matrix, calculates the bounding
 * rectangle and converts resulting doubles to integers so that points of the
 * target raster partially covered by resulting double points are included
 * to the resulting rectangle.
 *
 * Using this function instead of QWMatrix::map(const QRect&) eliminates many
 * artefacts and distortions seen in canvas views that use world transformation.
 * When scaling up, this function works merely as QWMatrix::map() with
 * transformation mode set to Areas, but it also produces correct rectangles
 * (as necessary to determine affected chunks) in all other cases. 
 */
static
QRect mapRectMax( const QWMatrix &m, const QRect &r )
{
    if ( m.isIdentity() ) {
        return r;
    }
    ...
    if ( m.m12() == 0.0 && m.m21() == 0.0 ) {
        // no shear or rotation, map two coords
        m.map( rect.left(), rect.top(), &x1, &y1 );
        m.map( rect.right(), rect.bottom(), &x2, &y2 );
    } else {
        double xl = rect.left(), yt = rect.top(); 
        double xr = rect.right(), yb = rect.bottom(); 
        double x, y;
        // map four coords and find min/max
        m.map( xl, yt, &x, &y );
        x1 = x;
        y1 = y;
        x2 = x;
    ...
    // get integers (partially covered points are included)
    x1 = floor( x1 );
    y1 = floor( y1 );
    x2 = ceil( x2 );
    y2 = ceil( y2 );
    ...
    // compose the result
    rect.setCoords( int( x1 ), int( y1 ), int( x2 ), int( y2 ) );
    // make bottom right inclusive again
    -- rect.rBottom();
    -- rect.rRight();
...
void QCanvas::drawViewArea( QCanvasView* view, QPainter* p, const QRect& vr, bool dbuf )
{
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
    QRect ivr = vr;
    if ( wm.m12() != 0.0 || wm.m21() != 0.0 ) {
        // Compensate for rounding errors happening somewhere on the path
        // update() -> drawViewArea() after many transformations (can be seen
        // when moving items in a canvas view that scales the canvas down
        // and rotates it to ~270 degress clockwise)
        ivr.addCoords( -1, -1, 1, 1 );
    }
    ivr = mapRectMax( iwm, ivr );
#else
	...
	if ( wm.m12()==0.0 && wm.m21()==0.0 && wm.m11() == 1.0 && wm.m22() == 1.0 )
	    ...
	    a = QPointArray( all );
#else
        // Polygons in OS/2 already include the right and bottom edges
        // (why shouldn't they do that?) Indeed, this important moment is not
        // defined in Qt at all. 
        QPointArray a( all );
#endif
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
        QWMatrix::TransformationMode oldMode = QWMatrix::transformationMode();
        QWMatrix::setTransformationMode( QWMatrix::Areas );
#endif        
	a = (wm*twm).map(a);
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
        QWMatrix::setTransformationMode( oldMode );
#endif        
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
		QRect r = changeBounds( mapRectMax( view->inverseWorldMatrix(),
                                        area ) );
#else
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
                    QRect vr = mapRectMax( wm, r );
                    if ( wm.m12() != 0.0 || wm.m21() != 0.0 ) {
                        // Compensate for rounding errors happening sometimes
                        // after two direct and inverse transformations
                        vr.addCoords( 0, 0, 1, 1 );
                    }
		    drawViewArea( view, &p, vr, dblbuf );
#else
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
    QRect safeArea = area;
    if (p && !p->worldMatrix().isIdentity()) {
        // Increase bounds that lay on the edges to get rid of drawing artefacts
        // caused by differeces in regions defined using 4 points and rectangles
        // drawn under the non-identity matrix
        if ( area.x() == 0 ) safeArea.rLeft() -= 2;
        if ( area.y() == 0 ) safeArea.rTop() -= 2;
        if ( area.right() == width() - 1 ) safeArea.rRight() += 2;
        if ( area.bottom() == height() - 1 ) safeArea.rBottom() += 2;
    }

    // Force the transformation mode to Areas. W/o this, drawBackground() and
    // drawForeground() will not fill the canvas correctly when scaling up
    // takes place.
    QWMatrix::TransformationMode oldMode = QWMatrix::transformationMode();
    QWMatrix::setTransformationMode( QWMatrix::Areas );
#endif
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
	drawBackground(painter,safeArea);
#else
	drawBackground(painter,area);
#endif
	allvisible.drawUnique(painter);
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
	drawForeground(painter,safeArea);
#else
...
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
	drawBackground(*p,safeArea);
#else
	drawBackground(*p,area);
#endif
	allvisible.drawUnique(*p);
#if !defined(QT_NO_CANVAS_ROUNDING_FIX)
	drawForeground(*p,safeArea);
#else
...
public:
	...
	for (int j=0; j<n; j++) {
	    ...
	    int x2r = x2%8;
#ifdef QCANVAS_POLYGONS_DEBUG
	    if ( dbg_ptr ) dbg_ptr->setPen(Qt::darkGray);
#endif
	    if ( x1q == x2q ) {
		...
		for (int i=x1q+1; i<x2q; i++) {
			...
			l[i]=0xff;
#ifdef QCANVAS_POLYGONS_DEBUG
        		if ( dbg_ptr ) dbg_ptr->setPen(Qt::black);
#endif
		    }
		}
...
#if defined(Q_WS_PM)
    // We cannot use QPolygonScanner, because:
    // a) polygons in OS/2 are all-inclusive, but
    // b) specifying the Bottom edge inclusion in QPolygonScanner::scan()
    //    doesn't actually work (seems that the code from Xserver always
    //    internally assumes that right and bottom edges are excluded).
    // Instead of getting into and fixing the Xserver code, I decided to use
    // region's rectangles to determine polygon spans. It should be as fast
    // as the Xserver code, but gives us one more advantage: spans detected
    // this way precisely repeat the actual polygon pixels drawn on the screen
    // (because both regions and polygons are created/drawn by GPI using the
    // same line and fill routines), so all chunks will be correctly detected.
    
    void scanPolygon( const QPointArray &pa, bool wind )
    {
	int cs = canvas->chunkSize();
        QRegion rgn = QRegion( pa, wind );
        QMemArray <QRect> rects = rgn.rects();
        for ( size_t i = 0; i < rects.size(); ++ i ) {
            const QRect &r = rects[i];
            int width = r.width() - 1;
            // I think I saw null rects in regions a couple of times...
...
#ifdef QCANVAS_POLYGONS_DEBUG
                if ( dbg_ptr ) {
                    dbg_ptr->setPen( Qt::cyan );
                    QPoint ptd = pt;
                    ++ ptd.ry();
                    int j = 1;
                    for ( ; j < cs && ptd.y() < r.bottom(); ++ j, ++ ptd.ry() )
                        dbg_ptr->drawLine( ptd, ptd + QPoint( width, 0 ) );
                }
#endif
                if ( pt.y() == r.bottom() )
                    break;
