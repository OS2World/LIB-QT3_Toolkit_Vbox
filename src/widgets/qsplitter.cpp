/****************************************************************************
** $Id: qsplitter.cpp 20 2005-11-17 18:00:27Z dmik $
**
...
void QSplitter::setRubberband( int p )
{
    ...
    int p1 = 0, p2 = 0, op1 = 0, op2 = 0, tmp = 0;
    if ( p >= 0 ) {
        p1 = p + hw / 2 - rBord;
        p2 = p1 + 2 * rBord - 1;
    }
    if ( opaqueOldPos >= 0 ) {
        op1 = opaqueOldPos + hw / 2 - rBord;
        op2 = op1 + 2 * rBord - 1;
        // exclude overlapping area to avoid flicker
        if ( p >= 0 ) {
            if ( p2 >= op1 && p2 <= op2 ) {
                tmp = op1;
                op1 = p2 + 1;
                p2 = tmp - 1;
            } else if ( p1 >= op1 && p1 <= op2 ) {
                tmp = op2;
                op2 = p1 - 1;
                p1 = tmp + 1;
            }
	...
	if ( opaqueOldPos >= 0 )
	    paint.drawRect( op1, r.y(), op2 - op1 + 1, r.height() );
	if ( p >= 0 )
	    paint.drawRect( p1, r.y(), p2 - p1 + 1, r.height() );
    } else {
	if ( opaqueOldPos >= 0 )
	    paint.drawRect( r.x(), op1, r.width(), op2 - op1 + 1 );
	if ( p >= 0 )
	    paint.drawRect( r.x(), p1, r.width(), p2 - p1 + 1 );
    }
    opaqueOldPos = p;
}
