/****************************************************************************
** $Id: qcolor.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
#define COLOR0_PIX (uint)CLR_FALSE
#define COLOR1_PIX (uint)CLR_TRUE
#else
#define COLOR0_PIX 0
#define COLOR1_PIX 1
#endif
...
#elif defined(Q_WS_PM)
        // the code below allows us to use negative color values (such as
        // CLR_WHITE) in range from -1 to -128 as direct pixel colors (i.e.
        // set using setPixel()) keeping their special meening.
        if ( d.d8.direct )
            return (int) ((char) d.d8.pix);
        else
            return d.d8.pix;
#else
	return d.d8.pix;
