/****************************************************************************
** $Id: qdns.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
#endif
#if defined(Q_OS_OS2)
//@@TODO (dmik): currently these headers are not used
//  (see comments to QDns::doSynchronousLookup())
...
#if defined(Q_OS_OS2)
//@@TODO (dmik): currently qdns_res_init() is not used
//  (see comments to QDns::doSynchronousLookup())
#else
...
{
#if defined(Q_OS_OS2)
    //@@TODO (dmik): we can use the code below on OS/2 also but this will
    //  require linking against the additional library containing res_*
    //  functions and global data, which is normally not used anywhere else
    //  (because QDns implementation does lookups diretcly to achieve
    //  asynchronous behavior) and will just waste some system resources.
    //  Anyhow, is synchronous behavior really necessary? Do nothing for now.
#else
    if ( t!=None && !l.isEmpty() ) {
	QValueListIterator<QString> it = n.begin();
	QValueListIterator<QString> end = n.end();
...
void QDns::doResInit()
{
    ...
    QFile resolvConf( QString( etc ) + "\\resolv" );
    if ( !resolvConf.exists() )
        resolvConf.setName( QString( etc ) + "\\resolv2" ); 
#else
...
#if defined(Q_OS_OS2)
//@@TODO (dmik): currently the code below is not used
//  (see comments to QDns::doSynchronousLookup())
#else
    if (ns->isEmpty()) {
