/****************************************************************************
** $Id: qcommonstyle.cpp 8 2005-11-16 19:36:46Z dmik $
**
				  ...
				  const QStyleOption& opt ) const
{
    ...
    case PE_CheckListController: {
        QRect pr( r );
        QPixmap px( (const char **)check_list_controller_xpm );
        // center pixmap if smaller, otherwise it will be scaled to fit
        int d = pr.width() - px.width();
        if ( d > 0 ) {
            pr.setX( pr.x() + d / 2 );
            pr.setWidth( px.width() );
        }
        d = pr.height() - px.height();
        if ( d > 0 ) {
            pr.setY( pr.y() + d / 2 );
            pr.setHeight( px.height() );
        }
	p->drawPixmap( pr, px );
	break; }
