# Qt project file
#
# :mode=makefile:tabSize=8:indentSize=4:noTabs=false:

TEMPLATE	= lib
TARGET		= qt
embedded:TARGET	= qte
VERSION		= 3.3.1
os2:VERSION	= $${VERSION}.0.7a
...
os2 {
	...
	TABLE_H     = $$OS2_ALL_H
	DIALOGS_H   = $$OS2_ALL_H
	ICONVIEW_H  = $$OS2_ALL_H
	NETWORK_H   = $$OS2_ALL_H
	OPENGL_H    = $$OS2_ALL_H
	TOOLS_H     = $$OS2_ALL_H
	CODECS_H    = $$OS2_ALL_H
	WORKSPACE_H = $$OS2_ALL_H
	XML_H       = $$OS2_ALL_H
	CANVAS_H    = $$OS2_ALL_H
	STYLES_H    = $$OS2_ALL_H
	CONFIG      -= incremental
	...
	!staticlib {
		DEFINES += QT_MAKEDLL
		exists(qt.rc):RC_FILE = qt.rc
		exists(qt.def.tpl.$$QMAKESPEC):DEF_FILE_TEMPLATE = qt.def.tpl.$$QMAKESPEC
		exists(qt.def.map.$$QMAKESPEC) {
			DEF_FILE_MAP = qt.def.map.$$QMAKESPEC
			debug {
				qt_def_map_debug.target = $(DEF_FILE_MAP)
				qt_def_map_debug.depends = $$DEF_FILE_MAP
				qt_def_map_debug.commands = $(COPY_FILE) "$$DEF_FILE_MAP" "$$qt_def_map_debug.target"
				QMAKE_EXTRA_TARGETS += qt_def_map_debug
				DEF_FILE_MAP = qt.def.map.$$(QMAKESPEC).debug
			}
		}
	}
}
...
shared:os2 {
	qt_official_build {
		TARGET = qt$$split(VERSION,.,)
		system( echo QMAKE_QT_DLL_TARGET = $$TARGET > ..\.qtbuild )
	} else {
		TARGET = $$QMAKE_QT_DLL_TARGET
	}
}
...
!os2:	include($$OPENGL_CPP/qt_opengl.pri)
...
os2 {
	# targets and commands to sync private headers
	private_headers.target = ..\.private_headers
	private_headers.depends = do_private_headers
	do_private_headers.target = do_private_headers
	do_private_headers.commands = cd .. && $(MAKE) private_headers
	QMAKE_EXTRA_TARGETS += do_private_headers private_headers
	PRE_TARGETDEPS += $$do_private_headers.target
	QMAKE_INTERNAL_INCLUDED_FILES += $$private_headers.target
}
