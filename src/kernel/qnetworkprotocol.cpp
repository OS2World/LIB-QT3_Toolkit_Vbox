/****************************************************************************
** $Id: qnetworkprotocol.cpp 8 2005-11-16 19:36:46Z dmik $
**
...
QNetworkProtocol::~QNetworkProtocol()
{
	...
	emit finished( op );
        // op->free() will be called by QNetworkOperationPrivate dtor below 
    }
    delete d;
}
...
{
    // ### A hackish workaround to prevent this op from being deleted during
    // the qApp->processEvents() call in QLocalFs::operationListChildren().
    // The proper way is to add something like QNetworkOperation::cancelFree()
    // to stop deleteTimer and make it a documented method.
    if ( !signalsBlocked() )
        delete this;
}
