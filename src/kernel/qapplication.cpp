/****************************************************************************
** $Id: qapplication.cpp 180 2008-03-01 15:42:34Z dmik $
**
...
/*!
  ...
  X-server.
  On Windows, OS/2 and Macintosh, currently the window system is always
...
QApplication::~QApplication()
{
    ...
    // make sure eventloop is cleaned up after all post-routines and some object
    // destructors (that may still use some eventloop services such as timers)
    // are called. Note that this place is still not perfect as destructors of
    // children of QApplication will be called (by the QObject dtor) after this
    // method anyway and nothing prevents them from using e.g. timers.
    if ( eventloop )
...
// this is temporarily disabled until most bugs are fixed,
// see http://svn.netlabs.org/qr3/ticket/9 for details
//#elif defined(Q_WS_PM)
//	    style = "Warp4";		// default style for OS/2
//#endif
...
//	     !(app_style = QStyleFactory::create( "Warp4" ) ) &&
...
/*!
  ...
  "compact". Depending on the platform, "windowsxp", "warp4", "aqua" or
 ...
 *****************************************************************************/
#ifndef QT_NO_SESSIONMANAGER
#if defined( QT_NO_SM_SUPPORT ) || defined( Q_WS_WIN ) || defined( Q_WS_MAC ) || defined( Q_WS_QWS ) || defined( Q_WS_PM )
