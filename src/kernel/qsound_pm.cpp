/****************************************************************************
** $Id: qsound_pm.cpp 144 2006-10-25 21:44:19Z dmik $
**
...
private:    
    ...
    QIntDict< QAuBucketMMPM > bucketMap;
    QPtrList< QAuBucketMMPM > bucketList;
...
#if defined(QT_DEBUG)
    QCString fileName() { return fName; };
#endif
...
void QAuBucketMMPM::open()
{
    ...
    if ( getenv( "QT_PM_NO_SOUND_SHARE" ) != NULL ) {
        ...
        // Too pity, but we have to use exclusive mode by default due to
        // problems with the uniaud driver that is most commonly used nowadays.
        // In non-exclusive (shared) mode, we will hang up in mmpm.dll doing
        // mciSendCommand( MCI_CLOSE ) in close() for the last played sound if
        // we start playing 10 or more sounds simultaneously (i.e. w/o waiting
        // until the previous one completely finishes playing).
    }
...
void QAuServerMMPM::serverUninit()
{
    ...
    for ( QAuBucketMMPM *b = bucketList.first(); b; b = bucketList.current() ) {
        QSound *s = b->sound();
        if ( s ) {
                                            ...
                                            MPARAM mp1, MPARAM mp2 )
{
    ...
    switch ( msg ) {
        ...
        case MM_MCINOTIFY: {
            ...
            QAuBucketMMPM *b = that->bucketMap.find( deviceId );
